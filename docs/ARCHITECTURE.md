...
```python
class CostTracker:
    """Track costs for LLM API calls and compute."""
    ...
    def track_llm_call(
        ...
        """Calculate and record LLM API cost."""
    ...
    def check_budget(self, execution_id: str) -> BudgetStatus:
        """Check if budget exceeded."""
        current = self.get_total_cost(execution_id)
        limit = self.get_budget_limit(execution_id)
        return BudgetStatus(
            current=current,
            limit=limit,
            exceeded=current > limit if limit else False,
        )
```
...
1. **State Rollback** (Internal)
```python
# Automatic snapshot before each step
snapshot = await state_manager.create_snapshot(execution_id)
...
2. **Compensating Transactions** (External)
```python
@dataclass
class CompensatingAction:
    """Action to undo a completed operation."""

    action_type: str
    undo_fn: Callable
    undo_args: Dict[str, Any]

# Example: Email sent - can't unsend, but can send correction
compensating_actions = []

# Send email
await email.send(to=user, subject="Important", body=message)
compensating_actions.append(
    CompensatingAction(
        ...
        undo_args={
            "to": user,
            "subject": "Correction",
            "body": "Please disregard previous email",
        },
    )
)

# On rollback
for action in reversed(compensating_actions):
    await action.undo_fn(**action.undo_args)
```
...
- [ADR-001: Choice of Python over Go](./adr/001-python-language.md) (coming soon)
- [ADR-002: PostgreSQL for state storage](./adr/002-postgres-state.md) (coming soon)
- [ADR-003: OpenTelemetry for observability](./adr/003-otel.md) (coming soon)
